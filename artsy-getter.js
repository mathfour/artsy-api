...
getList(getCountUrl, function (count) {
    ...
    for (var i = 0; i < stopAfter; i += maxRecordsAtATime) {
        ...
        .then(dealWithResponse).catch(function (error) {
...
async function dealWithResponse(response) {
    ...
    for (var k = 0; k < maxRecordsAtATime; k++) {
        ...
        let save = {
            ...
            follower_count: await scrapeIt(response.data._embedded.artists[k].slug)
            // bmc: will be scraped in just a sec...
        };
...
function getList(getCountUrlFocus, callback) {
    ...
    .then(function (response) {
        ...
        let totalRecordCount = JSON.stringify(response.data.total_count);
        ...
        let columnHeaders = 'id, slug, name, sortable_name, gender, created_at, updated_at, nationality, location, hometown \r';

        let useThisFileName = batchTitle + '_CurrentTotalCount' + totalRecordCount + '.' + fileType;
